...
import { CalendarRange, Download, ImageIcon, Trophy } from 'lucide-react';
...
export function SocialGraphicsWorkspace({ league, seasons }: Props) {
  ...
  const svgMarkup = useMemo(() => {
    ...
    switch (context) {
      case 'weekly-recap':
        return buildWeeklyRecapSvg({ league, season: selectedSeason, week: selectedWeek });
      case 'standings-update':
        return buildStandingsSvg({ league, season: selectedSeason, week: selectedWeek });
      case 'score-card':
      default:
        return buildScoreCardSvg({ league, season: selectedSeason, game: selectedGame, week: selectedWeek });
    }
  ...
  const downloadName = useMemo(() => {
    ...
    return `${base}-${season}-${context}${selectedWeek ? `-${selectedWeek.id}` : ''}.svg`;
  }, [context, league.name, selectedSeason?.name, selectedWeek]);
      ...
      <div className="space-y-5">
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-neutral-400">
            Pick a season, a week, and a graphic type. Every preview below is rendered from your actual completed games and standings.
          </p>
          ...
          <div className="mt-5 space-y-4">
            <Field label="Season">
              <select
                ...
                className="w-full rounded-2xl border border-white/10 bg-neutral-950 px-4 py-3 text-sm text-white outline-none transition focus:border-rink-400"
              >
                {seasons.map((season) => (
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 rounded-2xl border px-4 py-3 text-left transition ${
                        active
                          ? 'border-rink-400/60 bg-rink-500/10 text-white'
        ...
        <button
          ...
          disabled={!svgMarkup}
          className="inline-flex items-center justify-center gap-2 rounded-2xl border border-rink-400/30 bg-rink-500/10 px-4 py-3 text-sm font-semibold text-rink-100 transition hover:bg-rink-500/20 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <div className="overflow-hidden rounded-[28px] border border-white/10 bg-black/30">
          {svgMarkup ? (
            ...
            <div className="flex aspect-[4/5] items-center justify-center p-6 text-center text-sm text-neutral-500">
              No social graphic can be generated yet. Add completed games to this season first.
            </div>
          )}
        </div>
      </div>
...
function buildWeekOptions(games: SocialGame[]): WeekOption[] {
  ...
  for (const game of sorted) {
    ...
    const index = Math.max(0, Math.floor((current - anchor) / (7 * 24 * 60 * 60 * 1000)));
  ...
  return Array.from(buckets.entries())
    ...
    .map(([index, bucket]) => {
      ...
      return {
        ...
        games: [...bucket].sort((a, b) => new Date(b.scheduledAt).getTime() - new Date(a.scheduledAt).getTime()),
      };
    });
}
...
function buildWeeklyRecapSvg({ league, season, week }: { league: Props['league']; season: SocialSeason; week: WeekOption | null }) {
  ...
  const games = (week?.games ?? season.games).slice(0, 6);
  ...
  return wrapSvg(`
    ...
    <rect x="72" y="244" width="936" height="168" rx="32" fill="#020617" fill-opacity="0.55" stroke="#ffffff" stroke-opacity="0.08" />
    <text x="116" y="310" fill="#f8fafc" font-size="42" font-family="Inter, Arial, sans-serif" font-weight="800">${games.length} completed games</text>
    <text x="116" y="360" fill="#cbd5e1" font-size="28" font-family="Inter, Arial, sans-serif">${totalGoals} total goals this window${topTeam ? ` • ${escapeXml(topTeam.teamName)} leads with ${topTeam.points} pts` : ''}</text>

    ${games.map((game, index) => renderRecapRow(game, index)).join('')}

    <rect x="72" y="1138" width="936" height="142" rx="28" fill="#020617" fill-opacity="0.58" stroke="#ffffff" stroke-opacity="0.08" />
    <text x="112" y="1200" fill="#f8fafc" font-size="32" font-family="Inter, Arial, sans-serif" font-weight="700">Keep the league feed moving</text>
    <text x="112" y="1246" fill="#cbd5e1" font-size="25" font-family="Inter, Arial, sans-serif">Export this recap after each scoring window for a consistent weekly post cadence.</text>
  `);
}
...
function buildStandingsSvg({ league, season, week }: { league: Props['league']; season: SocialSeason; week: WeekOption | null }) {
  ...
  return wrapSvg(`
    ...
    <text x="670" y="297" fill="#94a3b8" font-size="24" font-family="Inter, Arial, sans-serif">PTS</text>
    <text x="792" y="297" fill="#94a3b8" font-size="24" font-family="Inter, Arial, sans-serif">REC</text>
    <text x="924" y="297" fill="#94a3b8" font-size="24" font-family="Inter, Arial, sans-serif">+/-</text>

    ${leaders.map((team, index) => renderStandingsRow(team, index)).join('')}

    <text x="72" y="1248" fill="#fef3c7" font-size="24" font-family="Inter, Arial, sans-serif">Top ${leaders.length} clubs by live standings table</text>
  `);
}
...
function renderRecapRow(game: SocialGame, index: number) {
  const y = 448 + index * 110;
  return `
    <rect x="72" y="${y}" width="936" height="88" rx="24" fill="#020617" fill-opacity="0.42" stroke="#ffffff" stroke-opacity="0.06" />
    <text x="108" y="${y + 54}" fill="#f8fafc" font-size="28" font-family="Inter, Arial, sans-serif" font-weight="700">${escapeXml(game.awayTeam?.name || 'Away')} ${game.awayScore}  •  ${game.homeScore} ${escapeXml(game.homeTeam?.name || 'Home')}</text>
    <text x="972" y="${y + 54}" text-anchor="end" fill="#94a3b8" font-size="22" font-family="Inter, Arial, sans-serif">${escapeXml(formatShortDate(game.scheduledAt))}</text>
  `;
}

function renderStandingsRow(team: SocialStanding, index: number) {
  ...
  return `
    ...
    <text x="108" y="${y + 51}" fill="#ffffff" font-size="28" font-family="Inter, Arial, sans-serif" font-weight="800">${team.rank}</text>
    <text x="188" y="${y + 51}" fill="#ffffff" font-size="28" font-family="Inter, Arial, sans-serif" font-weight="700">${escapeXml(team.teamName)}</text>
    <text x="682" y="${y + 51}" fill="#ffffff" font-size="28" font-family="Inter, Arial, sans-serif" font-weight="800">${team.points}</text>
    <text x="814" y="${y + 51}" fill="#e2e8f0" font-size="26" font-family="Inter, Arial, sans-serif">${team.wins}-${team.losses}-${team.ties}</text>
    <text x="968" y="${y + 51}" text-anchor="end" fill="#e2e8f0" font-size="26" font-family="Inter, Arial, sans-serif">${team.goalDiff >= 0 ? '+' : ''}${team.goalDiff}</text>
  `;
}
