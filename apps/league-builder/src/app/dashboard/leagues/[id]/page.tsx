          ...
          {league.seasons && league.seasons.length > 0 ? (
            <div className="grid gap-4 md:grid-cols-2">
              {league.seasons.map((season: any) => (
                <SeasonCard key={season.id} season={season} leagueId={leagueId} />
              ))}
            </div>
          ) : (
...
function SeasonCard({ season, leagueId }: { season: any; leagueId: string }) {
  ...
  return (
    <div className="bg-white/[0.04] border border-white/10 backdrop-blur-xl rounded-xl p-5 hover:border-white/20 transition-colors">
      ...
      <div className="flex items-center gap-2 mt-4">
        ...
        <Link
          href={`/dashboard/leagues/${leagueId}/seasons/${season.id}/edit`}
          className={cn(
