...
export async function requestAccountDeletion(reason?: string): Promise<{
  ...
  const stripeDataTyped = stripeData as any;
...
export async function getAccountDeletionStatus(): Promise<{
  ...
  const deletionLogData = deletionLog as any;
  const scheduledFor = new Date(deletionLogData.scheduled_for);
  const now = new Date();
  const daysRemaining = Math.ceil((scheduledFor.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));

  return {
    hasPendingDeletion: true,
    deletionRequest: deletionLogData as AccountDeletionRequest,
...
export async function getAllDeletionRequests(
  ...
  let query = (supabase.from('account_deletion_log') as any).select('*').order('requested_at', { ascending: false });
