...
export function suggestSwaps(divisions: DivisionHealth[]): SwapSuggestion[] {
  const suggestions: SwapSuggestion[] = [];

  // Only suggest swaps between divisions with outliers
  ...
  for (let i = 0; i < divsWithOutliers.length; i++) {
    for (let j = i + 1; j < divsWithOutliers.length; j++) {
      ...
      // Find the strongest outlier that should move up from the weaker division
      // and the weakest outlier that should move down from the stronger division
