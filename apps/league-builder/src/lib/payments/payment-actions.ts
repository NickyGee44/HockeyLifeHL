...
const ARCHIVABLE_PAYMENT_STATUSES = ['pending', 'overdue', 'cancelled', 'failed'] as const;
const ARCHIVE_REASON_CONFIRMATION = 'DELETE';
...
function isPaymentArchived(payment: Pick<PlayerPayment, 'archived_at'>) {
  return Boolean(payment.archived_at);
}

function getArchivedPaymentError(payment: Pick<PlayerPayment, 'archived_at'>) {
...
function canPermanentlyDeletePayment(
  payment: Pick<PlayerPayment, 'amount_paid_cents' | 'status' | 'archived_at'>,
  transactions: PaymentCleanupTransaction[],
  disputeCount: number
) {
