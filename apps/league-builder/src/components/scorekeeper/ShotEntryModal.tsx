...
export function ShotEntryModal({
  ...
  const t = useTranslations('scorekeeper.shotEntry');
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center">
      <div className="bg-neutral-900 w-full md:max-w-3xl md:rounded-2xl border-t md:border border-cyan-500/20 max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-neutral-800">
          <div>
            <h2 className="text-lg font-bold text-white">{t('recordShot')}</h2>
            <p className="text-sm text-neutral-400">
              {t('shootingOnDefending', { shooting: shootingTeam.name, defending: defendingTeam.name, period })}
            </p>
          </div>
          <button
            onClick={onCancel}
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={t('searchShooter', { team: shootingTeam.name })}
          ...
          <div>
            <p className="text-xs text-neutral-400 uppercase tracking-wider mb-2">{t('defendingGoalie')}</p>
            {goalies.length === 0 ? (
              <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-amber-100 text-sm">
                {t('noGoalieFound', { team: defendingTeam.name })}
              </div>
            ) : (
          ...
          <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-3">
            <p className="text-sm text-cyan-100 font-semibold">{t('shotSummary')}</p>
            <p className="text-sm text-neutral-300 mt-1">
              {t('shooterLabel')} {selectedShooter ? `#${selectedShooter.jerseyNumber} ${selectedShooter.fullName}` : t('teamShotLabel')}
            </p>
            <p className="text-sm text-neutral-300">
              {t('savedByLabel')} {selectedGoalie ? `#${selectedGoalie.jerseyNumber} ${selectedGoalie.fullName}` : t('notSelected')}
            </p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-neutral-800 flex gap-3">
          ...
          >
            {isSubmitting ? t('saving') : t('addShot')}
          </button>
        </div>
      </div>
    </div>
