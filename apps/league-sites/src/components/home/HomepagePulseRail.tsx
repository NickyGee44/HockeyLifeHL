...
import { CalendarDays, Camera, ChevronRight, Clock3, Sparkles } from 'lucide-react';
import { SocialLinks } from '@/components/SocialLinks';
import type { LeagueEvent, WebsiteSettings } from '@/lib/types';
...
function getCountdownParts(targetDate: string) {
  ...
  return {
    ...
    secondaryLabel: 'Hours',
    summary: days > 0 ? `${days} day${days === 1 ? '' : 's'} to go` : `${totalHours} hours to go`,
...
export function HomepagePulseRail({
  ...
  const nextEvents = futureEvents.slice(0, 2);
  const hasCalendar = futureEvents.length > 0;
  const countdownParts = countdown ? getCountdownParts(countdown.targetDate) : null;
  const showCommunityCard = !!photoHighlight || !!socialSettings;

  if (!countdownParts && !hasCalendar && !showCommunityCard) {
    return null;
  }

  const { monthLabel, cells } = buildCalendarMonth(futureEvents);
      ...
      {countdown && countdownParts && (
        <section className="glass-card rounded-[28px] px-5 py-5 md:px-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-[22px] border border-[var(--color-border)] bg-[var(--color-surface)]/72 p-4 text-center">
        ...
        <section className="glass-card rounded-[28px] px-5 py-5 md:px-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 text-xl font-black text-[var(--color-text-primary)]">Pulse Calendar</h3>
            </div>
            <div className="rounded-2xl bg-[var(--league-primary)]/12 p-2.5 text-[var(--league-primary)]">
          ...
          <div className="mt-4 rounded-[26px] border border-[var(--color-border)] bg-[var(--color-surface)]/72 p-4">
            <div className="flex items-center justify-between gap-3">
          ...
          {nextEvents.length > 0 && (
            <div className="mt-4 space-y-2">
              {nextEvents.map((event) => (
                <div
                  key={event.id}
                  className="flex items-center justify-between gap-3 rounded-[22px] border border-[var(--color-border)] bg-[var(--color-surface)]/62 px-3 py-3"
                >
        ...
        <section className="glass-card relative overflow-hidden rounded-[28px]">
          <div className="relative h-full min-h-[320px]">
            {photoHighlight.imageUrl ? (
              ...
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,color-mix(in_srgb,var(--league-primary)_26%,transparent),transparent_62%),linear-gradient(145deg,color-mix(in_srgb,var(--league-primary)_16%,transparent),var(--color-surface))]" />
            )}
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.08)_0%,rgba(0,0,0,0.54)_48%,rgba(0,0,0,0.88)_100%)]" />
            <div className="absolute inset-x-0 bottom-0 p-5">
              ...
              <p className="mt-2 text-sm text-white/78">{photoHighlight.subtitle}</p>
        ...
        <section className="glass-card rounded-[28px] px-5 py-5 md:px-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 text-xl font-black text-[var(--color-text-primary)]">Follow The League</h3>
            </div>
            <div className="rounded-2xl bg-[var(--league-primary)]/12 p-2.5 text-[var(--league-primary)]">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">
            Photos, reactions, and quick updates keep the homepage feeling current between games and stories.
          </p>
          <div className="mt-5 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-surface)]/65 p-4">
