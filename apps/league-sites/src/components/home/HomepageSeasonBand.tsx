...
function MiniTeamLogo({ name, logoUrl }: { name: string; logoUrl?: string | null }) {
  return (
    <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]/85">
      {logoUrl ? (
...
function RegistrationCard({ spotlight }: { spotlight: RegistrationSpotlight }) {
  ...
  return (
    <section className="league-shell-panel rounded-[30px] border border-[var(--color-border)] px-5 py-5 md:px-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--league-primary)]">
            Registration Timeline
          </p>
          <h3 className="mt-2 text-2xl font-black tracking-tight text-[var(--color-text-primary)]">
      ...
      <div className="mt-5 rounded-[26px] border border-[var(--color-border)] bg-[var(--color-surface)]/72 p-4">
        ...
        <div className="relative mt-3 h-2 rounded-full bg-[var(--color-surface-hover)]">
          <div
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 text-center">
          <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]/75 px-3 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-muted)]">Open</p>
            <p className="mt-2 text-sm font-bold text-[var(--color-text-primary)]">{formatShortDate(spotlight.opensAt)}</p>
          </div>
          <div className="rounded-2xl border border-[var(--league-primary)]/28 bg-[var(--league-primary)]/10 px-3 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--league-primary)]">Live</p>
            <p className="mt-2 text-sm font-bold text-[var(--color-text-primary)]">Register</p>
          </div>
          <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]/75 px-3 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-muted)]">Close</p>
            <p className="mt-2 text-sm font-bold text-[var(--color-text-primary)]">{formatShortDate(spotlight.closesAt)}</p>
          </div>
        </div>
      </div>
      ...
      <Link
        href={spotlight.href}
        className="mt-5 inline-flex items-center gap-2 rounded-full bg-[var(--league-primary)] px-4 py-3 text-sm font-bold text-[var(--color-accent-text)] transition-transform duration-200 hover:-translate-y-0.5"
      >
        Register Now
...
function ResultsCard({ leagueSlug, spotlight }: { leagueSlug: string; spotlight: ResultsSpotlight }) {
  return (
    <section className="league-shell-panel rounded-[30px] border border-[var(--color-border)] px-5 py-5 md:px-6">
      ...
      <div className="mt-5 space-y-3">
        {spotlight.games.slice(0, 2).map((game) => (
          <Link
            ...
            className="flex items-center gap-3 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-surface)]/72 px-4 py-4 transition-colors duration-200 hover:border-[var(--league-primary)]"
          >
            ...
            <div className="text-center">
              <p className="text-xl font-black leading-none text-[var(--color-text-primary)]">{game.away_score ?? '-'}</p>
              <p className="mt-1 text-[10px] uppercase tracking-[0.14em] text-[var(--color-text-muted)]">AT</p>
              <p className="mt-1 text-xl font-black leading-none text-[var(--color-text-primary)]">{game.home_score ?? '-'}</p>
            </div>
      ...
      <Link
        href={spotlight.href}
        className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[var(--league-primary)]"
      >
        All Scores
...
function GalleryCard({ spotlight }: { spotlight: GallerySpotlightCard }) {
  return (
    <section className="relative overflow-hidden rounded-[30px] border border-[var(--color-border)] bg-[var(--color-surface)]">
      ...
      <div className="relative flex min-h-[280px] flex-col justify-end p-5 md:p-6">
        ...
        <h3 className="mt-3 text-2xl font-black tracking-tight text-white">{spotlight.highlight.title}</h3>
        <p className="mt-2 max-w-sm text-sm leading-6 text-white/78">{spotlight.highlight.subtitle}</p>
        <Link
          href={spotlight.highlight.href}
          className="mt-5 inline-flex w-fit items-center gap-2 rounded-full bg-white px-4 py-3 text-sm font-bold text-slate-950 transition-transform duration-200 hover:-translate-y-0.5"
        >
...
export function HomepageSeasonBand({
  ...
  return (
    <section className="container mx-auto px-4 pt-6">
      <div className={`grid gap-6 ${spotlight && hasLeaders ? 'xl:grid-cols-[340px_minmax(0,1fr)]' : ''}`}>
        {spotlight &&
        ...
        {hasLeaders && (
          <section className="league-shell-panel rounded-[30px] border border-[var(--color-border)] px-5 py-5 md:px-6 md:py-6">
            <HomepageLeadersTabs
