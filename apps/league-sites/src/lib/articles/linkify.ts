...
function addMention(mentions: Map<string, ArticleMention>, mention: ArticleMention) {
...
function getGamePhrases(game: ArticleLinkableGame) {
  const home = normalizeWhitespace(game.homeTeamName);
  const away = normalizeWhitespace(game.awayTeamName);
...
export function buildArticleMentions(args: {
  ...
  for (const game of args.games) {
    const href = `/${args.leagueSlug}/games/${game.id}`;
    for (const phrase of getGamePhrases(game)) {
      addMention(mentions, {
        text: phrase,
        href,
        kind: 'game',
