-- ==============================================================================
-- MULTI-INSTANCE ARCHITECTURE - CORRECTED VERSION
-- ==============================================================================
...
CREATE INDEX IF NOT EXISTS idx_leagues_status_subscription ON leagues(status, subscription_tier);
...
DECLARE
  ...
  subdomain_part TEXT;
BEGIN
  -- Remove port and www prefix, normalize to lowercase
  ...
  -- Return empty if no match (platform domain)
  RETURN;
END;
$$;
...
SELECT '=== Testing domain lookup function ===' as info;
...
SELECT '=== League branding data ===' as info;
SELECT id, name, slug, subdomain, primary_color, secondary_color, accent_color FROM league_branding;

-- Show all columns added
...
WHERE table_name = 'leagues'
  AND column_name IN ('subdomain', 'accent_color', 'banner_url', 'favicon_url', 'font_family', 'custom_css', 'tagline', 'custom_domain_verified', 'is_public')
ORDER BY column_name;

SELECT '✅ Multi-instance architecture applied successfully!' as status;
