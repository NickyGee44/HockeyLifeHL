-- Also respect legacy player_availability rows that explicitly mark a player out.
...
appearances AS (
  ...
  WHERE tr.status = 'active'
    ...
    AND NOT EXISTS (
      ...
      WHERE pa.game_id = g.id
        AND pa.team_id = tr.team_id
        AND pa.player_id = tr.player_id
        AND pa.status = 'out'
    )
  ...
  WHERE si.status = 'accepted'
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM public.player_availability pa
      WHERE pa.game_id = si.game_id
        AND pa.team_id = si.team_id
        AND pa.player_id = si.invited_player_id
        AND pa.status = 'out'
    )
