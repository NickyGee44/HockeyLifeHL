-- ==============================================================================
-- MIGRATION 7 - FINAL WORKING VERSION
-- ==============================================================================
-- Fixes:
-- 1. Handles duplicate NULL seasons/teams
-- 2. Uses correct enum value: 'owner' instead of 'admin'
-- 3. Gives ALL existing users access to League #1 (since it's the original data)
-- ==============================================================================
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      WHILE EXISTS (SELECT 1 FROM seasons WHERE name = new_name) LOOP
...
-- ==============================================================================
-- STEP 6: Create League Memberships for ALL Existing Users
-- ==============================================================================
DO $$
DECLARE
  legacy_league_id UUID := 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'::uuid;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '👤 Step 6: Creating league memberships for ALL existing users...';
  RAISE NOTICE '  (This is the original single-tenant data, so everyone gets access)';

  FOR user_record IN SELECT id, role FROM profiles
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM league_memberships
      WHERE league_id = legacy_league_id AND user_id = user_record.id
    ) THEN
      -- Map profile role to league membership role
      -- profile.role = 'owner' → league membership 'owner'
      -- profile.role = 'captain' → league membership 'admin'
      -- profile.role = 'player' → league membership 'member'
      IF user_record.role = 'owner' THEN
        assigned_role := 'owner';
      ELSIF user_record.role = 'captain' THEN
        assigned_role := 'admin';
      ELSE
      ...
      VALUES (legacy_league_id, user_record.id, assigned_role, 'active', NOW());
      membership_count := membership_count + 1;
    END IF;
  END LOOP;

  IF membership_count > 0 THEN
    RAISE NOTICE '  ✅ Created % league memberships', membership_count;
  ELSE
    RAISE NOTICE '  ✅ League memberships already exist';
  END IF;
END $$;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '============================================================================';
  RAISE NOTICE '✅ MIGRATION 7 COMPLETE (FINAL WORKING VERSION)';
  RAISE NOTICE '============================================================================';
  RAISE NOTICE '';
