...
$content = Get-Content -Path "src\types\database.ts" -Encoding Unicode -Raw
...
# Append the types
$newContent = $content + $types

# Write back as UTF-8
Set-Content -Path "src\types\database.ts" -Value $newContent -Encoding UTF8
