...
export async function getAllSeasons() {
  try {
    ...
    return { error: error.message || 'Unauthorized', seasons: [] };
  }
}
...
export async function getActiveSeason() {
  try {
    ...
    return { error: error.message || 'Unauthorized', season: null };
  }
}
...
export async function getSeasonById(seasonId: string) {
  try {
    ...
    const { data: season, error } = await supabase
      ...
      .select("*")
      .eq("id", seasonId)
      .eq('league_id', leagueId) // CRITICAL: Verify season belongs to league
      .single();
...
export async function createSeason(formData: FormData): Promise<SeasonActionResult> {
  ...
  // Check for existing active season (within this league)
  if (setActive) {
  ...
  // Check for existing draft season (within this league)
  if (newStatus === "draft") {
  ...
  const { data: season, error } = await supabase
    .from("seasons")
    .insert({
      league_id: leagueId, // CRITICAL: Associate season with league
      name: name.trim(),
  ...
  // Import all existing teams for this season (filtered by league)
  const { data: allTeams } = await supabase
    .from("teams")
    .select("id")
    .eq('league_id', leagueId); // CRITICAL: Only teams from this league
  ...
  return { success: true, season };
    ...
    return { error: error.message || 'Unauthorized or failed to create season' };
  }
}
...
export async function updateSeason(seasonId: string, formData: FormData): Promise<SeasonActionResult> {
  ...
  return { success: true, season };
    ...
    return { error: error.message || 'Unauthorized or failed to update season' };
  }
}
...
export async function fixMultipleActiveSeasons(): Promise<SeasonActionResult> {
  try {
    ...
    const { data: activeSeasons, error: fetchError } = await supabase
      ...
      .eq('league_id', leagueId) // Filter by league
      .in("status", ["active", "playoffs"])
      .order("start_date", { ascending: false });
  ...
  return {
    ...
    console.error("Error in fixMultipleActiveSeasons:", error);
    return { error: error.message || 'Unauthorized or failed to fix seasons' };
  }
}
...
export async function updateSeasonStatus(seasonId: string, status: SeasonStatus): Promise<SeasonActionResult> {
  ...
  if (status === "active" || status === "playoffs") {
    ...
    if (allActiveSeasons && allActiveSeasons.length > 0) {
      ...
      const { data: seasonToActivate } = await supabase
        ...
        .select("id, name, start_date")
        .eq("id", seasonId)
        .eq('league_id', leagueId) // Verify season belongs to league
        .single();
  ...
  return { success: true, season };
    ...
    return { error: error.message || 'Unauthorized or failed to update season status' };
  }
}
...
export async function deleteSeason(seasonId: string): Promise<SeasonActionResult> {
  ...
  const { error } = await supabase
    ...
    .delete()
    .eq("id", seasonId)
    .eq('league_id', leagueId); // CRITICAL: Only delete seasons in this league
  ...
  return { success: true };
    ...
    return { error: error.message || 'Unauthorized or failed to delete season' };
  }
}
...
export async function incrementGameCount(seasonId: string): Promise<SeasonActionResult> {
  try {
    // This can be called by any league member (e.g., after game completion)
    const { leagueId } = await requireLeagueRole(['owner', 'admin', 'captain', 'scorekeeper']);
  ...
  if (fetchError || !season) {
    return { error: "Season not found or does not belong to your league" };
  }
  ...
  return { success: true };
    ...
    console.error("Error in incrementGameCount:", error);
    return { error: error.message || 'Unauthorized or failed to increment game count' };
  }
}
...
export async function archiveSeason(seasonId: string): Promise<SeasonActionResult> {
  const { leagueId } = await requireLeagueRole(['owner']);
